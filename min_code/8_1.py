...
def inp():
    ready_list = list(map(int, input().split()))
    for i in range(2):
        for j in range(3):
            x[i].append(ready_list[j+i*3])

def process():
    return sum(sum(i) for i in x)
def out(t):
    print(t)
...
def starBox():
    print(" ".join(f'{i*2+1}' for i in range(10)))
def macDoll():
    print(" ".join(chr(i) for i in range(72,64,-1)))
def copyBean():
    print(" ".join(f'{i}' for i in range(-5,6)))

coffee_price = int(input())

if 3500 <= coffee_price <= 5000:
    starBox()
elif 2500 <= coffee_price < 3500:
    macDoll()
else:
    copyBean()
