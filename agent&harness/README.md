...
- [Jira 연동과 규칙 문서 설계 사례](#jira-연동과-규칙-문서-설계-사례)
- [PRD 작성](#prd-작성)
- [Spec 작성](#spec-작성)
- [Phase 분할](#phase-분할)
...
## Jira 연동과 규칙 문서 설계 사례

실제 팀 프로젝트에서 `AGENTS.md` 같은 공통 규칙 파일을 어떻게 구체적으로 채웠는지 정리한다. 아래 예시는 실제 프로젝트 이름, 이슈 번호, 파일명을 일반화한 형태이다. 핵심은 규칙을 "설명"으로 남기지 않고, Agent가 그대로 실행할 수 있는 포맷·명령·쿼리 형태로 문서화하는 것이다.

### 규칙 문서를 3개 층으로 구성
- Git 브랜치/커밋 컨벤션
- Git 협업 워크플로우 규칙 (스펙 주도 개발, Jira 연동, MR 규격)
- 보안/자격 증명 관리 규칙

각 층은 서술형 문장 대신, 그대로 복사해 쓸 수 있는 포맷 규칙과 실제 예시를 함께 둔다.

```md
## 1. Git 브랜치 및 커밋 컨벤션
- 브랜치 형식: `타입/이슈번호-짧은설명` (kebab-case, 이슈 번호 필수)
- 커밋 형식: `타입(scope): 작업내용 (#이슈번호)`
- 타입: feat, fix, docs, refactor, chore
- Scope: be, fe, ai, infra
- 예시 브랜치: `feat/PROJ-123-login-api`
- 예시 커밋: `feat(be): 로그인 API JWT 토큰 발급 구현 (#PROJ-123)`
```

"형식 + 실제 예시"를 같이 두면 Agent가 컨벤션을 추측하지 않고 그대로 따라 쓴다.

### Jira를 정적 문서가 아니라 동적 쿼리로 연동
Jira 이슈 번호를 브랜치/커밋에 넣으라고만 적으면, Agent는 이슈가 이미 있다고 가정하거나 번호를 임의로 지어낼 수 있다. 이를 막기 위해 작업 시작 전 절차를 규칙에 명시했다.

```text
작업 시작
-> JQL로 관련 이슈가 이미 있는지 검색
-> 없으면 새 이슈 생성
-> 이슈 생성 시 상위 Epic과 현재 활성 Sprint를 JQL로 동적으로 조회해 연결
-> Epic/Sprint 연결을 필수로 만들어 이슈가 백로그에 방치되지 않게 한다
```

- 기존 이슈 검색과 신규 이슈 생성은 Jira 연동 MCP 도구(JQL 검색, 이슈 생성 도구)로 수행한다.
- 신규 이슈를 만들 때는 아래 JQL로 연결 대상을 매번 다시 조회한다.
  - 진행 중인 Epic: `issuetype = Epic AND statusCategory != Done`
  - 활성 Sprint: `sprint in openSprints()`
- 핵심은 "이슈/에픽/스프린트 번호를 규칙에 하드코딩하지 않고, 매번 현재 상태를 쿼리해서 연결 대상을 찾는다"는 점이다. 프로젝트가 진행되며 Epic과 Sprint가 바뀌어도 규칙 문서 자체는 고칠 필요가 없다.

### MR/PR 본문까지 규격화
브랜치·커밋 규칙만 있으면 MR(PR) 제목이 자동 생성된 영문 브랜치명으로 남는 문제가 생긴다. 이를 막기 위해 push 시점에 명시적으로 옵션을 지정하도록 규칙에 못박았다.

```text
MR Title 규격: [타입] 작업 내용 한글 요약 (#이슈키)
예: [Fix] 배포 설정 경로 수정 (#PROJ-136)

push 시 필수 옵션:
-o merge_request.title="[타입] 한글 요약 (#이슈키)"
-o merge_request.create
-o merge_request.target=dev
-o merge_request.description="$desc"
```

- MR 본문은 저장소의 MR 템플릿 문구를 그대로 두지 않고, 실제 작업 내용과 이슈 번호로 치환해서 채우도록 규칙에 명시한다.
- 여러 줄 본문을 안전하게 넘기기 위해 쉘 멀티라인 변수(PowerShell `$desc = @"..."@`, Bash heredoc 등)를 쓰라고 구체적으로 지정하면, Agent가 따옴표 escape 문제로 명령을 실패시키는 일을 줄일 수 있다.

### 규칙 문서 자체의 보호 규칙
- `AGENTS.md`/`CLAUDE.md` 같은 규칙 파일은 "사용자의 명시적 요청이나 팀 합의 없이 Agent가 임의로 수정할 수 없다"는 조항을 규칙 맨 앞부분에 둔다.
- 규칙을 지키게 만드는 메타 규칙이 없으면, Agent가 작업 중 편의를 위해 규칙 파일 자체를 완화해버리는 경우가 생길 수 있다.

### 스펙 주도 개발을 강제하는 템플릿
"바로 코딩 금지(Vibe Coding 금지)"를 말로만 적기보다, 스펙 템플릿을 정해진 경로에 두고 그 템플릿을 채우는 것을 승인 절차의 입구로 삼았다.

```md
# 작업 명세서 (Task Specification)
> 코드를 작성하기 전, 이 템플릿을 복사해 새 명세서를 작성하고 사용자 승인을 먼저 받는다.
...
## 3. 아키텍처 / 데이터 흐름
- 필요 시 Mermaid 다이어그램으로 입력~출력 흐름을 표현
...
- 이 템플릿은 [PRD 작성](#prd-작성), [Spec 작성](#spec-작성) 원칙을 실제 파일 경로와 체크리스트로 구체화한 버전이라고 볼 수 있다.
- "변경 대상"에 추가/수정 파일을 미리 선언하게 하면, 리뷰 시 실제 diff가 선언한 범위를 벗어났는지 바로 대조할 수 있다.

### 문서 폴더를 역할별로 분리
- `specs/`: 코딩 전 작성하는 작업 명세 (위 템플릿의 결과물)
- `history/`: 과거 버전, 변환 전 원본, 폐기된 설계, 의사결정 기록
- `design/`: 디자인 시안과 프로토타입
- `members/`: 팀원별 학습·작업 기록

현재 팀이 참조해야 하는 "정본" 문서는 루트에 두고, 과거 자료는 `history/`로 옮긴다는 원칙을 문서 상단에 명시해 두면, Agent가 오래된 문서를 최신 기준으로 착각해서 참조하는 일을 줄일 수 있다.

### 이 사례에서 얻은 교훈
- 규칙은 "설명"보다 "그대로 실행 가능한 포맷·명령·쿼리"로 적을수록 Agent가 벗어날 여지가 줄어든다.
- 이슈·에픽·스프린트처럼 상태가 계속 바뀌는 대상은 값을 규칙에 박아두지 말고, 조회 쿼리를 규칙에 박아둔다.
- 브랜치·커밋·MR 컨벤션은 사람이 보기 좋은 문장이 아니라, push 명령의 옵션까지 못박아야 실제로 지켜진다.
- 규칙 파일과 스펙 템플릿의 경로를 공통 규칙 문서에 명시해 두면, 새 세션을 시작한 Agent도 같은 절차를 다시 찾을 수 있다.
