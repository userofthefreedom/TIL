...
[알고리즘](#알고리즘algorithm-기초)


# Python Study Notes
- 빠른 검색용 치트시트 (자주 찾는 것들)
  - 타입/변환
    - `type(x)` : 타입 확인
    - `isinstance(x, T)` : x가 T 타입(또는 하위 타입)인지 확인
    - `int(x) / float(x) / str(x) / list(x) / tuple(x) / set(x)` : 명시적 형변환
    - 예시
      ```py
  ...
  - 컬렉션 공용 함수
    - `len(iterable)` : 길이
    - `sum(nums)` : 합 (숫자)
    - `min(nums) / max(nums)` : 최소/최대
    - `sorted(iterable, key=..., reverse=...)` : 정렬된 **새 리스트** 반환
    - `any(cond for ...)` : 하나라도 True면 True
    - `all(cond for ...)` : 모두 True면 True
    - 예시
      ```py
      nums = [3, 1, 2]
      print(len(nums))                 # 3
      print(sum(nums))                 # 6
      print(min(nums), max(nums))      # 1 3
      ...
      ```
  - 정렬 패턴 (알고리즘에서도 자주 씀)
    - 리스트 메서드 `list.sort()` vs 내장 함수 `sorted()`
      - `sort()` : 원본 리스트를 정렬(제자리 정렬), 반환값 `None`
      - `sorted()` : 정렬된 새 리스트 반환, 원본 유지
    - `key=` : “비교 기준”을 뽑아주는 함수
    - 예시 (튜플 리스트를 두 번째 값 기준으로 정렬)
      ```py
      students = [("지민", 25), ("서준", 20), ("민우", 30)]

      students.sort(key=lambda x: x[1])
      print(students)
      # 출력: [('서준', 20), ('지민', 25), ('민우', 30)]

      students2 = sorted(students, key=lambda x: x[0])
      print(students2)
      # 출력: [('민우', 30), ('서준', 20), ('지민', 25)]
      ```
  - 얕은/깊은 복사 빠른 정리
    - 얕은 복사(Shallow): 바깥 껍데기만 새로 만들고, 안쪽(중첩)은 공유할 수 있음
    - 깊은 복사(Deep): 안쪽까지 전부 새로 복사
    - 자주 쓰는 복사 방법
      - `b = a[:]` / `b = a.copy()` : 얕은 복사
      - `import copy; b = copy.deepcopy(a)` : 깊은 복사
...
- PyCharm에서 Markdown 표가 깨질 때(특히 `|` 인라인 코드)
  - 증상: 표 셀 안의 `` `5 | 3` `` 같은 코드가 열 구분자로 오인되어 깨질 수 있음
  - 해결(안 깨지는 작성)
    - `|` 대신 HTML 엔티티 사용: `&#124;`
    - 예: `` `5 &#124; 3` `` 또는 표 바깥에 코드블록으로 분리
...
- 알고리즘을 공부할 때 “체크리스트”
  - 문제를 읽을 때
    - 입력 크기(N) 확인 → 대략적인 시간 복잡도 목표를 잡기
    - “정답이 여러 개?” / “최소/최대?” / “경우의 수?” / “경로?” 같은 키워드 체크
  - 설계할 때
    - 자료구조 선택: list / dict / set / stack / queue
    - 탐색 전략: 완전탐색 / DFS / BFS / 이진탐색
    - 설계 전략: 탐욕 / 분할정복 / DP
  - 구현할 때
    - 인덱스 범위(특히 2차원), 방문 체크, 종료 조건(재귀) 먼저
    - 디버깅은 “작은 입력” + “print/로그” + “중단점”으로

- Stack (스택) & 재귀 & DFS 연결 감각
  - 스택은 “되돌아가기(backtracking)”가 필요할 때 자연스럽게 등장
  - 재귀는 내부적으로 스택 프레임을 쌓는 것과 같음
  - DFS는 “스택(또는 재귀)”으로 구현하는 대표 탐색

- Memoization ↔ DP 연결 감각
  - Memoization(Top-Down): 재귀로 풀다가, 계산한 값을 저장해서 중복 제거
  - DP(Bottom-Up): 작은 값부터 표(dp 배열)를 채워 올라가기
  - 둘 다 핵심은 “중복 계산 제거 + 저장”
