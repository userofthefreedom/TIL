...
- 버블 정렬 (Bubble Sort)
  - 개념  
    - 인접한 두 값을 비교하여 큰 값을 뒤로 보내는 방식
    - 한 바퀴 돌 때마다 가장 큰 값이 맨 뒤로 이동
    - 리스트를 반복 순회하면서 인접 요소를 교환
      ```py
      arr = [5, 3, 2, 4]
      n = len(arr)
      ...
      ```
    - 프로세스 (배열: [5, 3, 2, 4])
      - 1패스 결과: [3, 2, 4, 5]  ← 5가 맨 뒤로 감
      - 2패스 결과: [2, 3, 4, 5]
  - 시간 복잡도: **O(N²)**
  - 특징
    - 장점: 구현 가장 단순
    - 단점: 교환이 많아 비효율적
    - 안정성: **안정 정렬(Stable)** (같은 값의 순서가 유지됨)
- 선택 정렬 (Selection Sort)
  - 개념  
    - 현재 위치(i)에 들어갈 **최솟값(또는 최댓값)을 선택**해서 한 번에 교환
    - 매 단계마다 최소값을 찾아 위치 교환
      ```py
      arr = [5, 3, 2, 4]
      n = len(arr)
      ...
      ```
  - 프로세스 (배열: [5, 3, 2, 4])
    - i=0: 최솟값 2 찾음 → 5와 교환 → [2, 3, 5, 4]
    - i=1: 남은 [3,5,4] 중 최솟값 3 → 제자리 → [2, 3, 5, 4]
    - i=2: 남은 [5,4] 중 최솟값 4 → 교환 → [2, 3, 4, 5]
  - 시간 복잡도: **O(N²)**
  - 특징
    - 장점: **교환 횟수가 적다** (각 단계 최대 1회 교환)
    - 단점: 비교는 항상 많이 함
    - 안정성: 보통 **불안정 정렬(Unstable)** (교환 때문에 같은 값 순서가 바뀔 수 있음)
- 삽입 정렬 (Insertion Sort)
  - 개념  
    - 앞쪽을 “이미 정렬된 영역”이라고 가정하고
    - 새 원소(key)를 그 안의 **올바른 위치에 끼워 넣는다**
      ```py
      arr = [5, 3, 2, 4]
      ...
      ```
  - 프로세스 (배열: [5, 3, 2, 4])
    - 장점: **거의 정렬된 배열에서는 매우 빠름(거의 O(N))**
    - 단점: 역순에 가까우면 많이 밀어야 해서 느림
    - 안정성: **안정 정렬(Stable)** (같은 값은 보통 순서 유지)
  - 시간 복잡도: **O(N²)**  
  - 특징: 거의 정렬된 상태에서는 빠름
- 병합 정렬 (Merge Sort)
  - 개념  
    - 데이터를 반으로 나누고, 정렬 후 다시 합치는 분할 정복 알고리즘
  - 원리  
    - 재귀적으로 나누고, 정렬된 두 배열을 병합
      ```py
      def merge_sort(arr):
          if len(arr) <= 1:
              return arr
...
- 퀵 정렬 (Quick Sort)
  - 개념  
    - 기준값(pivot)을 기준으로 작은 값과 큰 값을 분할
  - 원리  
    - 분할 정복 방식, 평균적으로 매우 빠름
      ```py
      def quick_sort(arr):
          if len(arr) <= 1:
              return arr
      ...
      print(quick_sort([5,3,2,4]))
      # 출력: [2, 3, 4, 5]
      ```
  - 시간 복잡도: 평균 **O(N log N)**, 최악 **O(N²)**
  - 특징: 실무/라이브러리에서 자주 사용되는 원리
